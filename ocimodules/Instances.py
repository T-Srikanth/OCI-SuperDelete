...
def DeleteClusterNetworks(config, Compartments):
    AllItems = []
    ...
    while itemsPresent:
        ...
        for item in AllItems:
            try:
                itemstatus = object.get_cluster_network(cluster_network_id=item.id).data
                if itemstatus.lifecycle_state != "TERMINATED":
                    if itemstatus.lifecycle_state != "TERMINATING":
                        try:
                            print ("Deleting: {}".format(itemstatus.display_name))
                            object.terminate_cluster_network(cluster_network_id=itemstatus.id)
                        except:
...
def DeleteVolumeGroups(config, Compartments):
    ...
    print ("Getting all Volume Group objects")
    for Compartment in Compartments:
...
def DeleteVolumes(config, Compartments):
    ...
    print ("Getting all Volume objects")
    for Compartment in Compartments:
